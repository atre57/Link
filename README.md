    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; text-decoration: none; list-style: none; outline: none; }
        ...
        #auth-screen {
            ...
            height: 100vh; width: 100%; position: fixed; z-index: 100000; 
            display: flex; align-items: center; justify-content: center;
        }
        
        .auth-container { 
            background-color: var(--color-dark); border-radius: 20px; 
            box-shadow: 0 15px 35px rgba(0,0,0,0.5); position: relative; 
            overflow: hidden; width: 768px; max-width: 95%; min-height: 480px; 
        ...
        .auth-input { 
            ...
            margin: 8px 0; padding: 12px 15px; font-size: 13px; border-radius: 8px; width: 100%; transition: 0.3s;
        }
        ...
        .checkbox-container {
            ...
            margin-top: 10px; color: var(--color-info-dark); font-size: 0.85rem; cursor: pointer;
        }
        .checkbox-container input { accent-color: var(--v-cyan); cursor: pointer; width: 16px; height: 16px; }
        ...
        .auth-btn { 
            ...
            transition: transform 0.2s, box-shadow 0.2s; border: none; width: 100%;
        }
        ...
        aside .sidebar a { 
            display: flex; color: var(--color-info-dark); padding: 1rem 1.5rem; 
        ...
        .chat-msg { background: rgba(255,255,255,0.05); padding: 8px 12px; border-radius: 8px; border-left: 3px solid var(--v-cyan); word-break: break-all;}
        ...
        .chat-input-area button { 
            ...
            transition: 0.3s; color: white; border: none; flex-shrink: 0;
        }
...
<body>
            ...
            <div class="form-container sign-in">
                <form>
                    ...
                    <label class="checkbox-container">
                        <input type="checkbox" id="rememberMe"> Beni Hatırla (Otomatik Giriş)
                    </label>

                    <button type="button" class="auth-btn" onclick="doLogin()">SİSTEME GİR</button>
                    <button type="button" class="auth-btn" onclick="doGuest()" style="background: rgba(255,255,255,0.1); color: #ccc; margin-top: 10px;">MİSAFİR GİRİŞİ</button>
                </form>
            </div>
            <div class="toggle-container">
                <div class="toggle">
                    <div class="toggle-panel toggle-left">
                        <h1>TEKRAR HOŞGELDİN</h1>
                        <p style="margin-bottom: 20px; font-size: 0.9rem; color: #eee;">Kaldığın yerden devam etmek için giriş yap.</p>
                        <button class="auth-btn" id="loginBtn" style="border: 1px solid #fff; background: transparent;">GİRİŞ YAP</button>
                    </div>
    ...
    <div id="dashboard">
        <aside>
            ...
            <div class="sidebar">
                ...
                <a onclick="logout()"><span class="material-icons-sharp">logout</span><h3>Sistemden Çıkış</h3></a>
            </div>
        </aside>
    ...
    <script>
        ...
        window.onload = () => {
            ...
            if(savedUser) {
                // Veritabanından hesabı ve VIP statüsünü doğrulayarak giriş yap (Yetki kaybı olmaması için)
                db.ref('users/' + savedUser).once('value').then(s => {
                    if(s.exists()) {
                        loginSuccess(savedUser, s.val().isVip, true); // true = zaten kayıtlı
                    } else {
        ...
        function doLogin() {
            ...
            if(u === "ADMIN" && p === "1200778") { loginSuccess(u, true, remember); return; }
            
            db.ref('users/'+u).once('value').then(s => {
                if(s.exists() && s.val().password === p) {
                    loginSuccess(u, s.val().isVip, remember);
                } else alert("Kullanıcı adı veya şifre hatalı!");
            });
        }

        function doGuest() { loginSuccess("MİSAFİR_" + Math.floor(Math.random()*1000), false, false); }

        function loginSuccess(u, vip, rememberMode) {
            sessionStorage.setItem("user", u);
            
            // Eğer "Beni Hatırla" seçildiyse tarayıcıya kaydet
            if(rememberMode) {
                localStorage.setItem("nexus_saved_user", u);
            }
        ...
        function logout() {
            ...
            localStorage.removeItem("nexus_saved_user"); // Otomatik girişi de iptal et
            location.reload();
        }

        // --- 2. DASHBOARD İŞLEVLERİ ---
        let myChartInstance;
        function initChart() {
            ...
            myChartInstance = new Chart(ctx, { 
                ...
                data: { 
                    labels: mockLabels, 
                    datasets: [{ 
                        label: 'Sistem Yükü (%)', data: mockData, 
        ...
        // Chat Sistemi (Mesajların kalıcılığı için limit artırıldı)
        function sendChat() { 
        ...
        // LIMIT 50'ye çıkarıldı. Mesajlar artık silinmeyecek, kalıcı olarak birikecek.
        db.ref('chat').limitToLast(50).on('value', s => { 
        ...
        function toggleMusic() { 
            ...
            if(isPlaying) m.pause(); else { m.volume = 0.3; m.play(); }
        ...
        function updatePShape(shape) {
            ...
            particles.forEach((p, i) => {
                let t = (i / numParticles) * Math.PI * 2; let x, y;
                ...
                if (shape === 'circle') { x = Math.cos(t) * scale; y = Math.sin(t) * scale; }
                    ...
                    if(side === 0) { x = -size/2 + (size * progress); y = -size/2; }
                    else if(side === 1) { x = size/2; y = -size/2 + (size * progress); }
                    else if(side === 2) { x = size/2 - (size * progress); y = size/2; }
                    else { x = -size/2; y = size/2 - (size * progress); }
                }
                else if (shape === 'star') {
                    let spikes = 5; let outerRadius = scale * 1.2; let innerRadius = scale * 0.5;
                    ...
                    let angle1 = rot + currentSpike * step; let angle2 = rot + (currentSpike + 1) * step;
                    let r1 = (currentSpike % 2 === 0) ? outerRadius : innerRadius;
                    let r2 = (currentSpike % 2 === 0) ? innerRadius : outerRadius;
                    let x1 = Math.cos(angle1) * r1; let y1 = Math.sin(angle1) * r1;
                    let x2 = Math.cos(angle2) * r2; let y2 = Math.sin(angle2) * r2;
                    x = x1 + (x2 - x1) * localProgress; y = y1 + (y2 - y1) * localProgress;
                }
